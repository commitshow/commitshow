...
interface GitHubInfo {
  ...
  signals: {
    ...
    // ── Build origin (internal analytics · NOT user-facing) ──
    // What generator / agent the user built this product with. Independent
    // axis from form_factor (which classifies the product shape) and
    // business_category (which classifies the product domain). Stored for
    // internal cohort analysis · partnership signals · marketing taxonomy.
    // DO NOT expose in ladder / project detail / share cards — this is a
    // private signal until we have explicit policy on surfacing it.
    build_origin:            'lovable' | 'v0' | 'bolt' | 'replit_agent' | 'cursor_assisted' | 'claude_code_assisted' | 'unknown'
    build_origin_evidence:   string      // human-readable evidence ('README: lovable.dev/projects/...')
...
async function inspectGitHub(url: string | null, explicitWorkspace: string | null = null): Promise<GitHubInfo> {
  ...
  // build_origin computed below after readmeRaw is fetched.
  let build_origin: 'lovable' | 'v0' | 'bolt' | 'replit_agent' | 'cursor_assisted' | 'claude_code_assisted' | 'unknown' = 'unknown'
  let build_origin_evidence = ''
  ...
  // build_origin · internal cohort signal (NOT user-facing · B2B / partnership
  // analytics use only). Footprint detection — what generator / AI agent the
  // user shipped with. Order: most specific first so 'lovable-tagger' dep
  // beats a generic 'README mentions cursor' hit. Weak 'assisted by'
  // signals (CLAUDE.md / .cursorrules) only fire when nothing stronger did
  // — those tell us the user uses the tool, not that the tool authored
  // the whole product.
  {
    const readmeHead = (readmeRaw ?? '').slice(0, 4000)
    const allDepsForOrigin = pkgParsed
      ...
      : {}
    const lovableTaggerDep = 'lovable-tagger' in allDepsForOrigin ||
      Object.keys(allDepsForOrigin).some(d => d.startsWith('@lovable/') || d.startsWith('lovable-'))
    const lovableReadme = /\b(lovable\.dev|generated by lovable|built with lovable|made with lovable|created with lovable)\b/i.test(readmeHead)
    const v0Readme      = /\bv0\.dev\/(r|chat)\/|generated by v0\.dev|v0 by vercel|built with v0\b/i.test(readmeHead)
    const boltReadme    = /\bbolt\.new\/~?\/|generated by bolt|built with bolt\.new\b/i.test(readmeHead)
    const replitReadme  = /\breplit agent\b|replit\.com\/@.+\/.+/i.test(readmeHead) ||
      rootPathSet.has('.replit')
    const cursorAssisted = rootPathSet.has('.cursorrules') ||
      rootPaths.some(p => /^\.cursor\/rules\//.test(p))
    const claudeCodeAssisted = rootPathSet.has('CLAUDE.md') ||
      rootPaths.some(p => /^\.claude\/(skills|commands|agents)\//.test(p))

    if (lovableTaggerDep)        { build_origin = 'lovable';              build_origin_evidence = 'lovable-tagger dep present' }
    else if (lovableReadme)      { build_origin = 'lovable';              build_origin_evidence = 'README mentions lovable.dev' }
    else if (v0Readme)           { build_origin = 'v0';                   build_origin_evidence = 'README mentions v0.dev' }
    else if (boltReadme)         { build_origin = 'bolt';                 build_origin_evidence = 'README mentions bolt.new' }
    else if (replitReadme)       { build_origin = 'replit_agent';         build_origin_evidence = '.replit or Replit URL in README' }
    else if (claudeCodeAssisted) { build_origin = 'claude_code_assisted'; build_origin_evidence = 'CLAUDE.md or .claude/{skills,commands,agents}/ present' }
    else if (cursorAssisted)     { build_origin = 'cursor_assisted';      build_origin_evidence = '.cursorrules or .cursor/rules/ present' }
  }
