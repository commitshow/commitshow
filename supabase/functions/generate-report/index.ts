...
async function buildDeep(period: string, asOf: string) {
  ...
  }).filter(s => s.n > 0).sort((a, b) => (b.fail_pct ?? -1) - (a.fail_pct ?? -1))
...
async function buildSurface(period: string, asOf: string) {
  ...
  }).sort((a, b) => (b.fail_pct ?? -1) - (a.fail_pct ?? -1))
...
Deno.serve(async (req) => {
  ...
  try {
    if (action === 'draft') {
      const period = String(p.period || '').match(/^[0-9a-z-]{4,12}$/) ? p.period : `${asOf.slice(0, 4)}-q${Math.floor((new Date().getUTCMonth()) / 3) + 1}`
      const trend = await buildTrend().catch(() => null)  // null until history accumulates
      const out: Record<string, unknown> = {}
      for (const kind of (p.kind ? [p.kind] : ['ai-built', 'web-security'])) {
        const row = kind === 'web-security' ? await buildSurface(period, asOf) : await buildDeep(period, asOf)
        ;(row as Record<string, unknown>).trend = trend
        const r = await upsert(row)
        out[kind] = r.ok ? { slug: row.slug, hero: (row.hero_stat as any)?.value } : { error: r.body }
      }
      return json({ period, trend: trend ? 'included' : 'no history yet', drafts: out })
    }
